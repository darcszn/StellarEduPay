...
async function syncPaymentsForSchool(school) {
  ...
  while (!done) {
    for (const tx of page.records) {
      ...
      if (feeValidation.status === 'underpaid') {
        ...
        await Payment.create({
          ...
          feeValidationStatus: 'underpaid', excessAmount: 0, status: 'FAILED',
          memo, senderAddress, isSuspicious: true,
          suspicionReason: feeValidation.message, ledger: txLedger,
          confirmationStatus: 'failed', confirmedAt: txDate,
        });
        continue;
      }
      ...
      await Payment.create({
        schoolId, studentId: intent.studentId, txHash: tx.hash,
        amount: paymentAmount, feeAmount: intent.amount,
        feeValidationStatus: cumulativeStatus, excessAmount, status: 'confirmed',
        memo, senderAddress, isSuspicious: collision.suspicious,
        suspicionReason: collision.reason, ledger: txLedger,
        confirmationStatus, confirmedAt: txDate,
      });

      logger.info('Transaction recorded', {
        txHash: tx.hash, schoolId, studentId: intent.studentId,
        amount: paymentAmount, feeValidationStatus: cumulativeStatus,
        isSuspicious: collision.suspicious, confirmationStatus,
      });
    ...
    if (!done) {
      ...
      page = await withStellarRetry(() => page.next(), { label: `syncPaymentsForSchool.next(${schoolId})` });
...
async function parseIncomingTransaction(txHash, walletAddress = null) {
  ...
  return { hash: tx.hash, successful: tx.successful, memo, payments, created_at: tx.created_at };
}
